    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        xg.setOnClickListener(view -> {
            if (xmm1.getText().equals(xmm2.getText())) {
                ...
                try {
                    jsonObject.put("oldPassword", jmm.getText());
                    jsonObject.put("newPassword", xmm1.getText());
                    new HttpUtil()
                            .sendResulToken("/prod-api/api/common/user/resetPwd", jsonObject.toString(), "POST", new Callback() {
                                @Override
