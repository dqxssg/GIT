...
/**
 * Servlet implementation class getRankByHospitalId
 */
@WebServlet("/getRankByHospitalId")
public class getRankByHospitalId extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public getRankByHospitalId() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		db.getRs("select hospitalId,rank from hospitalinfo where hospitalId=" + hospitalId);
		...
		try {
			if (set != null) {
				...
				while (set.next()) {
					...
					jsonObject3.put("rank",  set.getInt(2));
