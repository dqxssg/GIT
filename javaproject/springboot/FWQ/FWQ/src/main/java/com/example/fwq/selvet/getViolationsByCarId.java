...
/**
 * Servlet implementation class getViolationsByCarId
 */
@WebServlet("/getViolationsByCarId")
public class getViolationsByCarId extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public getViolationsByCarId() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		db.getRs("select * from violations where carid='" + carid + "'");
		...
		try {
			if (set != null) {
				...
				while (set.next()) {
					...
					jsonObject3.put("place", set.getString(4));
					jsonObject3.put("deductPoints", set.getInt(5));
					jsonObject3.put("cost", set.getInt(6));
					jsonObject3.put("status", set.getInt(7));
					jsonObject3.put("description", set.getString(8));
					jsonObject3.put("notification", set.getString(9));
