...
/**
 * Servlet implementation class getAllOrderType
 */
@WebServlet("/getAllOrderType")
public class getAllOrderType extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public getAllOrderType() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if (set != null) {
				...
				while (set.next()) {
					map.put(set.getString(2), set.getString(2));
				}
				for (String string : map.values()) {
					if (string == null || string.equals("")) {
						continue;
					}
					jsonObjects.add(string);
					jsonArray.add(string);
				}
				jsonObject2.put("ROWS_DETAIL", jsonArray.toString());
			} else {
