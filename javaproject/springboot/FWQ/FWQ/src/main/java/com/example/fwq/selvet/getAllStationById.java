...
/**
 * Servlet implementation class getAllStationById
 */
@WebServlet("/getAllStationById")
public class getAllStationById extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public getAllStationById() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		db.getRs("select * from subwaystation where id ='" + subwayID + "'");
		...
		try {
			if (set != null) {
				...
				while (set.next()) {
					...
					jsonObject3.put("stationIndex", set.getInt(3));
					jsonObject3.put("stationname", set.getString(4));
					jsonObject3.put("distance", set.getInt(5));
