...
/**
 * Servlet implementation class fpVillageList
 */
@WebServlet("/fpVillageList")
public class fpVillageList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		db.getRs("select * from fpvilliage");
		...
		try {
			if (set != null) {
				...
				while (set.next()) {
					...
					jsonObject3.put("villid", set.getInt(1));
					jsonObject3.put("villname", set.getString(2));
					jsonObject3.put("villpicture", urlString + "/images/" + set.getString(3));
					jsonObject3.put("villaddress", set.getString(4));
					jsonObject3.put("villdesc",  set.getString(5));
