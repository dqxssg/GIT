...
/**
 * Servlet implementation class getOrderByType
 */
@WebServlet("/getOrderByType")
public class getOrderByType extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		db.getRs("select * from order_t where type = '" + type + "'");
		...
		try {
			if (set != null) {
				...
				while (set.next()) {
					...
					jsonObject3.put("date", set.getString(3).replace(".0", ""));
					jsonObject3.put("cost", set.getFloat(4));
