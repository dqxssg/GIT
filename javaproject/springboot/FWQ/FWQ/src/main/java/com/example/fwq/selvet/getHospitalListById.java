...
/**
 * Servlet implementation class getHospitalListById
 */
@WebServlet("/getHospitalListById")
public class getHospitalListById extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public getHospitalListById() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		db.getRs("select * from hospital where hospitalId='"+hospitalId+"'");
		...
		try {
			if (set != null) {
				...
				while (set.next()) {
					...
					jsonObject3.put("hospitalName", set.getString(2));
					jsonObject3.put("picture", urlString + "/images/" + set.getString(3));
					jsonObject3.put("desc", set.getString(4));
					jsonObject3.put("icon", urlString + "/images/" + set.getString(5));
					jsonObject3.put("url", set.getString(6));
