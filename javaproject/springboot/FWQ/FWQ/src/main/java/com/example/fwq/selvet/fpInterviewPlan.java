...
/**
 * Servlet implementation class fpInterviewPlan
 */
@WebServlet("/fpInterviewPlan")
public class fpInterviewPlan extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String villiagerid = jsonobject.getString("villiagerid");
		...
		try {
			int row = db.update("insert into fpinterview (villiagerid,starttime,endtime,intent,userid) values('"+villiagerid+"','"+starttime+"','"+endtime+"','"+intent+"','"+userid+"')");
			if (row == 1) {
