...
/**
 * Servlet implementation class getActionCommitById
 */
@WebServlet("/getActionCommitById")
public class getActionCommitById extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public getActionCommitById() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		db.getRs("select * from actioncommit where id=" + id);
		...
		try {
			if (set != null) {
				...
				while (set.next()) {
					...
					jsonObject3.put("username", set.getString(3));
					jsonObject3.put("commitTime", set.getString(4).replace(".0", ""));
					jsonObject3.put("commitContent", set.getString(5));
