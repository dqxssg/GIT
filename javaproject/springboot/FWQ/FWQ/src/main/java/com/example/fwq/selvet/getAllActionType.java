...
/**
 * Servlet implementation class getAllActionType
 */
@WebServlet("/getAllActionType")
public class getAllActionType extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public getAllActionType() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		db.getRs("select * from actiontype");
		...
		try {
			if (set != null) {
				...
				while (set.next()) {
					...
					jsonObject3.put("typename", set.getString(2));
